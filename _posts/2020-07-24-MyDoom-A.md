...
The sample uses the function at ```0x4A465E```, it takes an address and an encrypted string. looking at the cross references to this function implies that it's definitely a string decoder as it gets called from too many placess across the binary
...
so i decoded the strings and added them as comments using IDAPython to speed up the reversing process
